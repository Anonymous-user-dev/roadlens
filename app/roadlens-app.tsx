...
export function RoadLensApp() {
  ...
  const routeIssues = useMemo(() => [...mapIssues].filter((issue) => issue.status !== "repaired").sort((a, b) => {
  ...
  function selectIssue(issue: Issue) {
    ...
    setMapMode("live");
    const canMap = issue.locationSource !== "approximate" && (issue.locationAccuracy === null || issue.locationAccuracy <= 100);
    setMapNotice(canMap ? `Focused on ${issue.id}` : `${issue.id} needs location verification before it can be mapped`);
    setMapCommand((current) => ({ kind: canMap ? "priority" : "city", sequence: current.sequence + 1 }));
  }
  ...
  async function analyze() {
    ...
    try {
      ...
      const result = await response.json() as { detections?: Array<{ confidence?: number; label?: string; box?: number[] }>; image?: { width: number; height: number } };
      ...
      const areaCoverage = best.box && best.box.length === 4 && result.image
        ? Math.max(0, Math.min(100, ((best.box[2] - best.box[0]) * (best.box[3] - best.box[1])) / (result.image.width * result.image.height) * 100))
        : null;
      const normalizedLabel = label.toLowerCase();
      const severity: Issue["severity"] = normalizedLabel.includes("pothole") && areaCoverage !== null && areaCoverage >= 8
        ? "Critical"
        : (normalizedLabel.includes("pothole") || normalizedLabel.includes("alligator")) && areaCoverage !== null && areaCoverage >= 2
          ? "High" : "Medium";
      const certainty: Detection["certainty"] = confidence >= 55 ? "probable" : "possible";
      const extent = areaCoverage === null ? "Its visible extent could not be measured reliably." : `Its detection box covers about ${areaCoverage.toFixed(1)}% of the photo.`;
      const explanation = `${certainty === "probable" ? "The road-distress model found a supported" : "The road-distress model found a lower-confidence"} ${label} pattern. ${extent} Priority is estimated from defect type and visible extent, then confirmed by a reviewer.`;
      setDetection({ confidence, severity, label, certainty, explanation, areaCoverage });
      ...
      <section className="workspace">
        <aside className="control-panel">
          ...
          <div className="issue-list">
            ...
            {issues.slice(0, 4).map((issue) => (
              <button key={issue.id} className={`issue-row ${selectedId === issue.id ? "active" : ""}`} onClick={() => selectIssue(issue)}>
        ...
        <section className="map-panel" aria-label="Dushanbe road issue map">
          ...
          {selected ? <article className="issue-detail">
            ...
            <Button className="route-button" disabled={!routeIssues.length} onClick={() => commandMap("route")}><Navigation /> Plan inspection</Button>
