...
import "./admin.css";
import { Activity, Check, ChevronRight, CircleAlert, ClipboardCheck, Clock3, Construction, Download, LayoutDashboard, LoaderCircle, MapPin, RefreshCw, Route, Search, ShieldCheck, Upload, Wrench, X } from "lucide-react";
...
const workflow: Array<{ status: WorkflowStatus; label: string }> = [
  // FRONTEND EDIT: This order controls the displayed workflow strip, not backend permissions.
...
function ReportEditor({ report, token, reviewer, working, onUpdate }: { report: ReviewReport; token: string; reviewer: string; working: boolean; onUpdate: (report: ReviewReport, update: Record<string, unknown>) => Promise<void> }) {
  ...
  const [repairSource, setRepairSource] = useState<string | null>(null); const [repairExists, setRepairExists] = useState(false); const [repairLoading, setRepairLoading] = useState(true); const [repairLoadError, setRepairLoadError] = useState(false); const [repairReload, setRepairReload] = useState(0); const [repairMessage, setRepairMessage] = useState(""); const [uploadingRepair, setUploadingRepair] = useState(false);
  const repairInput = useRef<HTMLInputElement>(null); const repairUrl = useRef<string | null>(null);
  const showRepair = useCallback((blob: Blob) => { if (repairUrl.current) URL.revokeObjectURL(repairUrl.current); repairUrl.current = URL.createObjectURL(blob); setRepairSource(repairUrl.current); }, []);
  useEffect(() => {
    const controller = new AbortController();
    fetch(`/api/review/repair-image?id=${encodeURIComponent(report.id)}`, { headers: { Authorization: `Bearer ${token}` }, cache: "no-store", signal: controller.signal })
      .then(async (response) => {
        if (response.status === 404) { setRepairExists(false); setRepairSource(null); return null; }
        if (!response.ok) throw new Error();
        return response.blob();
      })
      .then((blob) => { if (!controller.signal.aborted && blob) { showRepair(blob); setRepairExists(true); } })
      .catch(() => { if (!controller.signal.aborted) setRepairLoadError(true); })
      .finally(() => { if (!controller.signal.aborted) setRepairLoading(false); });
    return () => controller.abort();
  }, [repairReload, report.id, showRepair, token]);
  useEffect(() => () => { if (repairUrl.current) URL.revokeObjectURL(repairUrl.current); }, []);
  const patch = (values: Partial<Edits>) => setEdits((current) => ({ ...current, ...values }));
  const locationChanged = edits.latitude !== report.latitude || edits.longitude !== report.longitude;
  const correction = { street: edits.street, severity: edits.severity, defectType: edits.defectType, reviewerNote: edits.reviewerNote, ...(locationChanged ? { latitude: edits.latitude, longitude: edits.longitude } : {}) };
  ...
  async function uploadRepair(file?: File) {
    ...
    try { const response = await fetch("/api/review/repair-image", { method: "POST", headers: { Authorization: `Bearer ${token}` }, body: form }); const payload = await response.json() as { error?: string }; if (!response.ok) throw new Error(payload.error || "Completion photo could not be saved."); showRepair(file); setRepairExists(true); setRepairLoadError(false); setRepairMessage("Completion evidence attached."); }
  ...
  return <article className={`admin-detail status-${report.status}`}>
    ...
    <div className="admin-detail-body">
      ...
      {report.status !== "pending_review" && <div className="repair-evidence-control"><input ref={repairInput} hidden type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif" onChange={(event) => { const file = event.currentTarget.files?.[0]; event.currentTarget.value = ""; void uploadRepair(file); }} /><div><strong>Completion evidence</strong><span>{repairLoading ? "Checking completion evidence…" : repairExists ? "Ready for before-and-after verification" : repairLoadError ? "Could not check existing evidence" : "Required before closing this repair"}</span>{repairMessage && <em>{repairMessage}</em>}</div>{repairLoadError && <Button variant="outline" disabled={repairLoading} onClick={() => { setRepairLoading(true); setRepairLoadError(false); setRepairReload((value) => value + 1); }}><RefreshCw /> Retry</Button>}{report.status !== "repaired" && <Button variant="outline" disabled={uploadingRepair || repairLoading} onClick={() => repairInput.current?.click()}><Upload /> {uploadingRepair ? "Uploading…" : repairExists ? "Replace photo" : "Upload photo"}</Button>}</div>}
      <div className="review-actions">
        ...
        {next && <Button disabled={working || (next.status === "repaired" && (!repairExists || repairLoading))} onClick={() => onUpdate(report, { ...correction, status: next.status })}>{next.icon}{next.label}</Button>}
...
export default function ReviewPage() {
  ...
  async function updateReport(report: ReviewReport, update: Record<string, unknown>) {
    ...
    try {
      ...
      else setReports((current) => current.map((item) => item.id === report.id ? { ...item, ...update, status: payload.report!.status, locationSource: update.latitude !== undefined ? "reviewer" : item.locationSource, locationAccuracy: update.latitude !== undefined ? null : item.locationAccuracy, updatedAt: new Date().toISOString() } as ReviewReport : item));
  ...
  function exportCsv() {
    ...
    const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = `roadlens-reports-${new Date().toISOString().slice(0, 10)}.csv`; link.hidden = true; document.body.appendChild(link); link.click(); link.remove(); window.setTimeout(() => URL.revokeObjectURL(url), 1_000);
  }
  ...
  return <main className="review-shell">
    ...
    {!token ? <section className="review-login"><span className="review-login-icon"><ShieldCheck /></span><h1>Admin workspace</h1><p>Review road evidence, correct locations, prioritize repairs, and track work through completion.</p><form onSubmit={(event) => { event.preventDefault(); if (accessCode.trim() && reviewer.trim()) void loadQueue(accessCode.trim()); }}><label htmlFor="reviewer-email">Operator email</label><input id="reviewer-email" type="email" autoComplete="email" value={reviewer} onChange={(event) => setReviewer(event.target.value.slice(0, 120))} required /><label htmlFor="review-code">Admin access code</label><input id="review-code" type="password" autoComplete="current-password" value={accessCode} onChange={(event) => setAccessCode(event.target.value)} required /><Button type="submit" disabled={state === "loading"}>{state === "loading" ? "Opening…" : "Open admin panel"}</Button></form>{state === "error" && <div className="review-message error"><CircleAlert /> {message}</div>}</section>
      : <section className="admin-workspace">
        {/* FRONTEND EDIT: Admin heading and export/refresh actions. */}
        ...
        {/* FRONTEND EDIT: KPI cards. Move whole <article> elements to change their order. */}
        ...
        {/* FRONTEND EDIT: Queue is the first column and inspector is the second.
            Swap the child divs to reverse them, then adjust .admin-console in admin.css. */}
        ...
        {/* FRONTEND EDIT: Move this whole section above .admin-console to show recent activity earlier. */}
